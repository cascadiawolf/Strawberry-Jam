module.exports = function ({ dispatch, application }) {
  ...
  const sequenceFns = [
    r => `%xt%o%qx%${r}%%`,
    r => `%xt%o%qj%${r}%denTEST%20%1%0%`,
    r => `%xt%o%qs%${r}%dentestofpower%`,
    r => `%xt%o%qpup%${r}%key_3c%523273%`,
    r => `%xt%o%qat%${r}%trapkey_3%0%`,
    r => `%xt%o%qat%${r}%cosmo_4%0%`,
    r => `%xt%o%qaskr%${r}%cosmo_4%1%1%`,
    r => `%xt%o%qaskr%${r}%cosmo_4%1%1%`,
  ...
  dispatch.onCommand({
    ...
    callback: async () => {
      if (loopActive) {
        stopAll();
        log('warn', 'meetcosmo mode disabled.');
      } else {
        await startLoop();
        if (loopActive) {
          sniffing = true;
          log('success', 'meetcosmo enabled: Packet loop active and prize filtering on.');
        }
      }
    }
  });
